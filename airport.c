...
pthread_mutex_t mutex[11] = PTHREAD_MUTEX_INITIALIZER;
...
void *handle_plane_departure(void *m) {
	...
	//837
	/*printf("Departing plane");
    	printf("Arrival: %d\n", msg.arr);
	printf("Departure: %d\n", msg.dep);
	printf("Passenger/Cargo ID: %d\n", msg.pId);
	printf("Passenger/Cargo Type: %d\n", msg.pType);
	printf("Occupied Seats/Items: %d\n", msg.n);
...
int main(){
	...
	/*for (int i = 0; i < 11; i++) {
        printf("%d ", p.runwayWeights[i]);
    }*/
	
	//printf("\n%ld %d %d\n" , msg.mtype , msg.arr, msg.dep);
